...
class SteamTopSellers(Star):
    ...
    async def get_steam_top_sellers(self, event: AstrMessageEvent, args: str = ""):
        ...
        try:
            ...
            logger.debug(f"top_sellers: {top_sellers}")
            if not top_sellers:
            ...
            logger.error(
                "插件 [SteamTopSellers] 解析JSON失败。Steam返回的不是有效的JSON。"
            )
            response_text = await response.text() if response else "No response object"
            logger.error(f"收到的HTML响应开头: {response_text[:500]}")
            yield event.plain_result(
                "无法解析Steam返回的数据，服务器可能返回了HTML页面。"
            )
        except aiohttp.client_exceptions.ClientError as e:
            logger.error(f"插件 [SteamTopSellers] 请求Steam API失败: {e}")
            yield event.plain_result("网络请求失败，无法获取Steam热销榜。")
        except Exception as e:
            logger.error(
                f"插件 [SteamTopSellers] 处理数据时发生未知错误: {e}", exc_info=True
            )
            yield event.plain_result("处理数据时发生未知错误，请联系管理员。")
